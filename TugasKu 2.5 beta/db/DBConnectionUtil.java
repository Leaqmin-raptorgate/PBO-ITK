...
public class DBConnectionUtil {
    ...
    public static void initializeDatabase() {
        ...
        try (Connection conn = getConnection(); Statement stmt = conn.createStatement()) {
            ...
            // ===== Preload jadwal table with hari (Senin to Minggu) and sesi (1-4) =====
            if (dbNeedsInitialization) {
                String[] days = {"Senin", "Selasa", "Rabu", "Kamis", "Jumat", "Sabtu", "Minggu"};
                for (String hari : days) {
                    for (int sesi = 1; sesi <= 4; sesi++) {
                        String checkSql = "SELECT COUNT(*) FROM jadwal WHERE hari = ? AND sesi = ?";
                        try (PreparedStatement checkStmt = conn.prepareStatement(checkSql)) {
                            checkStmt.setString(1, hari);
                            checkStmt.setInt(2, sesi);
                            ResultSet rs = checkStmt.executeQuery();
                            if (rs.next() && rs.getInt(1) == 0) {
                                String insertSql = "INSERT INTO jadwal (hari, sesi) VALUES (?, ?)";
                                try (PreparedStatement insertStmt = conn.prepareStatement(insertSql)) {
                                    insertStmt.setString(1, hari);
                                    insertStmt.setInt(2, sesi);
                ...
                System.out.println("- Preloaded jadwal table with Senin to Minggu and sesi 1-4.");
            }
