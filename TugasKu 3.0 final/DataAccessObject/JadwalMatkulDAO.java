...
public class JadwalMatkulDAO {
    ...
    // Update the id_jadwal for a given id_jm.
    public boolean updateJadwalForJm(int idJm, int newIdJadwal) {
        String sql = "UPDATE jadwal_matkul SET id_jadwal = ? WHERE id_jm = ?";
        try (Connection conn = DBConnectionUtil.getConnection();
             PreparedStatement pstmt = conn.prepareStatement(sql)) {
            pstmt.setInt(1, newIdJadwal);
            pstmt.setInt(2, idJm);
            int affectedRows = pstmt.executeUpdate();
            return affectedRows > 0;
        } catch (SQLException e) {
