...
public class DetailTugasPanel extends javax.swing.JPanel {
    ...
    private void jButton2ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButton2ActionPerformed
        ...
        // === NEW FEATURE: Confirm if deadline is in the past ===
        java.time.LocalDate deadline = new java.sql.Date(date.getTime()).toLocalDate();
        java.time.LocalDate today = java.time.LocalDate.now();
        if (deadline.isBefore(today)) {
            int confirm = JOptionPane.showConfirmDialog(
                this,
                "Apakah anda yakin mengisi tugas yang telah lewat deadline?",
                "Konfirmasi Deadline Lewat",
                JOptionPane.YES_NO_OPTION
            );
        if (confirm != JOptionPane.YES_OPTION) {
            JOptionPane.showMessageDialog(this, "Telah dibatalkan");
            return;
        }
    }
    // === END NEW FEATURE ===
